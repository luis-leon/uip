...
2.	Se debe mantener el estandar para el nombrado de paquetes y clases.
	...
	gt.gob.sat.microservicio_solicitudesdiva				->	clases que permiten la ejecucipn del proyecto, no debe modificarlas.
	gt.gob.sat.microservicio_solicitudesdiva.controllers	->	clases que definen los endpoints de los servicios REST (ClaseController.java).
	gt.gob.sat.microservicio_solicitudesdiva.model			->	clases que contienen el mapeo de tablas (NombreDeTablaSinGuiones.java).
	gt.gob.sat.microservicio_solicitudesdiva.repositories	->	clases que permiten realizar las operaciones de persistencia o consultas a base de datos (ClaseRepository.java).
	gt.gob.sat.microservicio_solicitudesdiva.services		->	clases con la implementacion de la funcionalidad o logica de negocio (ClaseService.java).
