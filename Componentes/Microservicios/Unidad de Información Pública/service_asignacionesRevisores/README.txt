...
2.	Se debe mantener el estandar para el nombrado de paquetes y clases.
	...
	gt.gob.sat.uip.service_asignacionesrevisores				->	clases que permiten la ejecucipn del proyecto, no debe modificarlas.
	gt.gob.sat.uip.service_asignacionesrevisores.controllers	->	clases que definen los endpoints de los servicios REST (ClaseController.java).
	gt.gob.sat.uip.service_asignacionesrevisores.model			->	clases que contienen el mapeo de tablas (NombreDeTablaSinGuiones.java).
	gt.gob.sat.uip.service_asignacionesrevisores.repositories	->	clases que permiten realizar las operaciones de persistencia o consultas a base de datos (ClaseRepository.java).
	gt.gob.sat.uip.service_asignacionesrevisores.services		->	clases con la implementacion de la funcionalidad o logica de negocio (ClaseService.java).
